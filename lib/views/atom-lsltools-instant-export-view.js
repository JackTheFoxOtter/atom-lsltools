...
export default class LSLToolsInstantExportView {
    ...
    enter(callbackSelected, callbackCancelled) {
        ...
        var directoryPath = atom.config.get("LSL-Tools.pathToViewerTempFolder");
        var directory = new Directory(directoryPath);

        if (myself.dirOnChangeDisposable === undefined) {
            myself.dirOnChangeDisposable = directory.onDidChange(() => {
                myself.scanDirectory(directory);
            });
        }
